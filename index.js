...
app.post("/add-karyawan", (req, res) => {
  ...
  let insertQuery = `Insert into karyawan values (null,${db.escape(
    nama
  )}, ${db.escape(usia)}, ${db.escape(email)}, ${db.escape(berat)}, ${db.escape(
    kota
  )}, ${db.escape(tahun)}, ${db.escape(idposisi)} )`; //script query in mysql
  ...
  db.query(insertQuery, (err, results) => {
    ...
    db.query(
      ...
      (err2, results2) => {
        ...
        res
          .status(200)
          .send({ message: "Berhasil menambah data karyawan", data: results2 });
      }
    );
  });
});

//PATCH DATA TO MYSQL using request params
app.patch("/edit-karyawan/:id", (req, res) => {
  let patchData = [];
  for (let prop in req.body) {
    patchData.push(`${prop} = ${db.escape(req.body[prop])}`); //save data from req.body to an array
  }
  let editQuery = `update karyawan set ${patchData} where idkaryawan = ${db.escape(
    req.params.id
  )}`;
